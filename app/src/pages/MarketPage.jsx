...
export default function MarketPage() {
  ...
  // Conservative defaults sourced from 2025-26 roofing / home-services benchmarks (see README).
  const [radius, setRadius] = useState(15);
  const [impressionShare, setImpressionShare] = useState(40);
  ...
  return (
    <>
      <div className="market-subhdr">
        <div className="sub">
          Compare <b>Orlando</b> vs <b>Tampa Bay</b>. Generated {new Date(data.generated_at).toLocaleDateString()}. Source: Google Ads via DataForSEO.
        </div>
      ...
      <div className="layout">
        ...
        <aside className="panel">
          <MetricsPanel
            metro={metro} radius={radius}
            impressionShare={impressionShare} ctr={ctr} cvr={cvr}
            position={position}
          />
          ...
          <p className="footnote">
            ...
            Sliders default to the conservative side of published 2025-26 roofing / home-services benchmarks —
            replace with account actuals when available.
...
function MetricsPanel({ metro, radius, impressionShare, ctr, cvr, position }) {
  ...
  const cpcDelta = (other.blendedCPC != null && m.blendedCPC != null)
    ? other.blendedCPC - m.blendedCPC : null;

  return (
    <>
      <div className="metric hero">
        <div className="label">Cost per click (CPC)</div>
        <div className="value">{money(m.blendedCPC)}
          <span className="unit"> · {position === "top" ? "position #1" : "position 3–4"}</span></div>
        {cpcDelta != null && Math.abs(cpcDelta) >= 0.5 &&
          <div className="note">
            {position === "top"
              ? <>Owning #1 costs <span className="delta">+{money(-cpcDelta)}</span> per click vs position 3–4.</>
              : <>Owning #1 would cost <span className="delta">+{money(cpcDelta)}</span> per click.</>}
          </div>}
      </div>

      <div className="metric hero">
        <div className="label">Cost per lead (CPL)</div>
      ...
      <div className="metric">
        ...
        <div className="note">≈ {num(m.leadsMonth)} leads/month at {cvr}% CVR.</div>
      </div>
    </>
  );
}
...
function computeMetrics({ metro, radius, impressionShare, ctr, cvr, position }) {
  ...
  return {
    ...
    leadsMonth, leadsPerDay, costPerLead,
